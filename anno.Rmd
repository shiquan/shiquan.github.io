...
``` bash
PISA anno -gtf gencode.v44.annotation.gtf.gz -exon -vcf var.vcf.gz -vcf-ss -ref-alt -o anno.bam Parent_SC3v3_Human_Glioblastoma_possorted_genome_bam.bam 
```
...
PISA anno  -vcf var.vcf.gz -vcf-ss -ref-alt -o anno2.bam anno1.bam
```
**Notes:**

  - `-exon`: Generates exon and junction names.
  - `-vcf-ss`: Specifies strand information for each genetic variant, necessary because 10X scRNA-seq uses a strand-specific library.
