...
format:
  html:
    code-overflow: wrap
---
This vignette demonstrates how Yano can annotate genetic variants using public databases and assist users in interpreting the functional impact of these variants. The pathogenic mutations in single cells may help understand gene function or lineage tracing, but they are not relevant to the allele specific expression. So this vignette reuses the data but is independent from [allele specific gene expression from scRNA-seq](Yano_ASE.Rmd).

## 1. Predict the consequence of genetic variants

Load the object which was generated from [allele specific gene expression from scRNA-seq](Yano_ASE.Rmd). In that case, we used annoVAR to annotate the gene and approximate gene region type for the variants. In this section, we will go further, to predict the consequence of genetic variants. Note that, different from genetic annotation at DNA level, strand information is also considered here, so antisense terms will also be generated.

The predicted consequences were derived from transcript annotations. However, GENCODE annotations, generated as part of the ENCODE project, contain numerous truncated transcript records that are not fully consistent with the reference genome, which can introduce bias when predicting amino acid changes. For gene expression analysis, such as scRNA-seq, it is more common to use GENCODE, considering its good coverage. But for clinical research, validated annotations like RefSeq are preferred for genetic variant annotation. In this analysis, RefSeq annotations were downloaded from the UCSC server to provide more accurate and reliable predictions. Furthermore, the human genome reference is also needed for the prediction.
```bash
...
```
Map the consequence of genetic variants to Manhattan plot just for visualization but the p-value bears no relation to the consequence.
...
# remember to index this database, otherwise annoVAR will report an error.
...
We need to create new chromosome names that are compatible with the NCBI database format, because chromosome names in the ClinVar database look like '1', '2'.
```{r}
...
df %>% filter(CLNSIG %in% c('Pathogenic', 'Pathogenic/Likely_pathogenic', 'Pathogenic|risk_factor')) %>% DT::datatable()
```

In the selected SNVs, we found some reference alleles are highlighted, such as chr15:92992967C=/+. These reference alleles are more likely to be benign alleles. Let's query the original record in the ClinVar database to check what the exact situation is.
...
Unfortunately, many databases distributed in VCF format may contain inconsistently formatted tags. Apart from manually updating these databases or filtering reference alleles afterward, there is no straightforward solution to this issue.
...
```
Note that not all reads from this gene cover the SNV, which results in the SNV expression being sparser compared to the overall gene expression. So it is recommended to visualize both alleles for the same location together.
