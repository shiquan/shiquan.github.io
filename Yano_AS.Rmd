...
*Note:* Since the spatial dissimilarity test is not rely on cell clustering so changing the resolution or other parameters for `FindClusters` and `RunUMAP` will not impact the outcome of the spatial dissimilarity test.
```{r warning=FALSE}
...
obj[['exon']][[]] %>% filter(gene_name.padj<1e-8) %>% rownames -> exons
obj[['exon']][[]] %>% filter(gene_name.padj<1e-8) %>% pull(gene_name) -> bind.genes
