---
title: Alternative splicing analysis for scRNA-seq
date: 2024-07-08
format:
  html:
    code-overflow: wrap
...
dim(exp)
```
In this section, we will perform the standard Seurat analysis pipeline. Since our spatial dissimilarity test does not require predefined cell groups, we only need to calculate the shared nearest neighbors (SNN) using the `FindNeighbors` function. However, cell clustering and UMAP plots will still be generated to help interpret the results. Note that changing the resolution or other parameters for `FindClusters` and `RunUMAP` will not impact the outcome of the spatial dissimilarity test.
```{r warning=FALSE}
...
# IMPORTANT
# The permutation process is computationally expensive. In the example below, I set `perm=20` to only perform 20 permutations. However, the default setting performs 100 permutations for better evaluation. It's possible to increase the number of permutations even further, but it may not be necessary.
...
p2 <- FeaturePlot(obj, features = c("chr12:56161387-56161575/+/MYL6"), order = TRUE, pt.size = 2) & scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdBu")))
p1 + p2
```
