...
Sorted and indexed bam can be further used to do a lot of secondary analysis, but it's not mandatory if you just want the gene expression count matrix. Because annotation and expression count step do not require a sorted bam. However, the `corr` step will generate non-robust results if records are not sorted.
