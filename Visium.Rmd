...
obj[['exon']][[]] %>% filter(gene_name.padj<1e-10) %>% DT::datatable()
```
...
We previously used PCA to construct the cell weight matrix, but for spatial transcriptomics data, it is also possible to use the spatial coordinates, the real spot-to-spot distances, to build the weight matrix. Here, we demonstrate how to do this.

Starting from the latest version, Yano supports building the weight matrix from **multiple slices simultaneously**. When multiple image names are passed to `RunAutoCorr()`, `GetWeightsFromSpatial()` computes the spatial weight matrix for each image individually and then merges them into a single combined weight matrix via `mergeMatrix()`. This allows the spatial correlation analysis to leverage all slices together without needing to subset or analyze each slice separately.

```{r warning=FALSE, fig.width=12, fig.height=5}
# Use spatial coordinates from all four slices
obj <- RunAutoCorr(obj, image = c("slice1", "slice2", "slice3", "slice4"))
grep("_wm", names(obj), value=TRUE)

obj <- RunSDT(obj, bind.name = "gene_name", bind.assay = "RNA", wm.name = "spatial_wm")
FbtPlot(obj, val = "gene_name.padj")
Meta(obj) %>% filter(gene_name.padj<1e-10) %>% DT::datatable()

# SpatialFeaturePlot can display the results on any individual slice
SpatialFeaturePlot(obj, features = c("chr11:30106785-30109152/-/Sptbn1","Sptbn1"), images = c("slice1", "slice2", "slice3", "slice4"))
```
